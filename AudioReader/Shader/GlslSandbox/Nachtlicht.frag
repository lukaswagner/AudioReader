...
void main( void ) {
    vec2 uv = (texcoord * size + offset) * vec2(0.36, 1.8);
    float f = turb( 5.*uv ) * 1.5 + 0.3;
    f = clamp(f, 0.0, 1.0);
    f = smoothstep(0.6, 0.8, f);
    vec3 baseColor = vec3(0.07, 0.9, 0.4);//hsv
    vec3 c = hsv2rgb(baseColor) * f;
    color = vec4(c, 1.0); 
}
